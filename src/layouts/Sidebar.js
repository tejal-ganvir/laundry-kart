      ...
      <MenuList>
          <Link to={`/account/dashboard`} className={styles.sidebar_menu_item}>
            <MenuItem sx={{p: 2, textAlign:'center'}} onClick={() => setMenuSelected('Dashboard')}>
                <ListItemIcon >
                  <Avatar alt="Angelina Jolie" src={userImg} sx={{height:100 , width: 100}} />
                </ListItemIcon>
          ...
          <Divider />
        {
          customerMenu.map((item, idx) => (
            <Link key={`menu-item${idx}`} to={item.link} className={`${styles.sidebar_menu_item} ${styles.active}`}>
              <MenuItem sx={{p: 2}} selected={menuSelected === item.label} onClick={() => setMenuSelected(item.label)}>
                  <ListItemIcon >
                    {item.icon}
                  </ListItemIcon>
